...
install_country_instance() {
    echo -e "${YELLOW}Installing core via official SpherionOS installer...${NC}"
    ...
    # Stop native service if created
    ...
    # Find valid config file generated by original installer
    CONF_FILE=""
    for f in /etc/psiphon/psiphon.config /root/PsiphonLinux/psiphon.config /etc/psiphon.config; do
        if [ -f "$f" ]; then
            CONF_FILE="$f"
            break
        fi
...
EOF
        ...
        # Modify existing valid config using jq
        jq --arg country "$C_CODE" --argport "$PORT" '.EgressRegion = $country | .LocalSocksProxyPort = ($port | tonumber)' "$CONF_FILE" > "${CONF_FILE}.tmp" && mv "${CONF_FILE}.tmp" "$CONF_FILE"
    fi
...
[Service]
Type=simple
WorkingDirectory=$(dirname "$PSIPHON_BIN")
